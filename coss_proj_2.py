...
"""
2023 COSS 숙명여자대학교 데이터 서포터즈 송유빈
### 기획의도
- 최근 휴가철을 맞아 여행 계획을 알아보던 중, 국내 숙박업소 위치 비율/코로나19가 숙박업계에 미친 영향 등 다양한 방법으로 숙박업 데이터를 분석하고 싶다는 생각이 들어 해당 프로젝트를 선정하게 되었다.

### 이용 데이터
- 공공데이터 포털의 '행정안전부_숙박업' 데이터 (csv, https://www.data.go.kr/data/15044968/fileData.do)
- KOSIS 국가 통계 포털의  '지역별 면적' 데이터 (csv, https://kosis.kr/statHtml/statHtml.do?orgId=101&tblId=DT_1ZGA17)

### 데이터 전처리
1. 숙박업 데이터의 위치 좌표가 위/경도가 아닌 중부원점 기준 좌표로 설정
-> pyproj의 Proj, transform 모듈을 사용하여 변환
"""
...
"""2. 영업중인 숙박업소/폐업 상태인 숙박업소를 각각 다른 분석에 이용할 예정이기 때문에 데이터셋을 분리"""
df_open = df_new.loc[df['영업상태명']!='폐업']
df_close = df_new.loc[df['영업상태명']=='폐업']

st.code("""df_open = df_new.loc[df['영업상태명']!='폐업']
df_close = df_new.loc[df['영업상태명']=='폐업']""")

"""### 가설 설정 및 분석"""

st.write('<strong style="font-size:15pt;">가설 1. 영업중인 숙박업소는 부산/제주도 등 여행지에 가장 많이 분포할 것이다.</strong>', unsafe_allow_html=True)
"""- seaborn, folium 라이브러리를 이용한 시각화"""
...
따라서, 국내외에서 가장 여행을 많이 오는 곳인 서울시에 숙박업소가 가장 많이 분포되어 있고, """

st.write('<strong style="font-size:15pt;">가설2. 코로나19 이후로 숙박업체의 연평균 폐업률이 늘어났을 것이다.</strong>', unsafe_allow_html=True)
...
"""### Future work
- 숙박업소의 분포를 볼 때, 도 단위 말고 시 단위로 분석을 진행하면 더 정확한 분석 내용을 도출할 수 있을 것으로 기대된다.
- 코로나19가 숙박 업계에 큰 영향을 미치지는 않은 것으로 보여서, 최근 몇 년간 숙박업소의 폐업률이 높았던 다른 사회적 이유에 대해 알아볼 필요가 있어 보인다.
- 2003년에 압도적인 비율로 폐업 업소 수가 많았는데, 사회적 배경을 파악할 필요가 있어 보인다.
"""
